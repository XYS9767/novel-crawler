...
def decode_char(char_code):
    ...
    Args:
        char_code: 字符的 Unicode 编码
        
    Returns:
        解码后的字符
    """
    if CODE_ST <= char_code <= CODE_ED:
        bias = char_code - CODE_ST
        if 0 <= bias < len(CHARSET):
            mapped_char = CHARSET[bias]
            if mapped_char != '?':
                return mapped_char
...
def decode_text(encoded_text):
    ...
    for char in encoded_text:
        char_code = ord(char)
        decoded_char = decode_char(char_code)
...
if __name__ == '__main__':
    # 测试解码
    test_text = '集挖尊骨，二集，石村，复仇，镇压'
    print(f'原文：{test_text}')
    
    decoded = decode_text(test_text)
    print(f'解码：{decoded}')
    
    # 测试更多文本
    test2 = '我的兴趣爱好打王者荣耀'
    print(f'\n原文：{test2}')
    print(f'解码：{decode_text(test2)}')
